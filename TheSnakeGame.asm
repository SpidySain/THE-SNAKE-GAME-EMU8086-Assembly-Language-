...
speed_counter db 15         
base_speed    db 30          
return_msg db 10,13,13,10,10,13,9,9,9, "Press any key to return to MENU...", '$'
...
info3  db 16," Score: $"
info4  db 16," High Score: $"        
info5  db 16,"Good Luck!",2,"$"
...
gameover              db 13,10,10,10,10,10,10,10,9,9,"  ___   __   _  _  ____     __   _  _  ____  ____ ", 13,10 
              ...
              db 0ah,09h,09h,09h,"   High Score: ",0dh,0ah    
...
table1:   
    ...
    mov byte ptr [speed_counter], 10
    mov byte ptr [base_speed], 10
    ...
    ; Print high score immediately after drawing table
    mov dl,72
    mov dh,14
    mov ah,02h
    int 10h
...
dec byte ptr [speed_counter]     ;Speed Control
cmp byte ptr [speed_counter], 0
jne skip_movement          

; Reset speed counter based on current score (higher score = faster)
mov al, [score]            
mov bl, [base_speed]       
sub bl, al                
cmp bl, 1                 
...
gameover_screen:
    ...
    ; ------------------ Save last score into high_score unconditionally ------
    mov al,[score]
    mov [high_score], al
    ...
    ; -----------Prompt and wait for a key, then go back to menu (do NOT terminate program) ---------------
    lea dx, return_msg
    mov ah,9
    int 21h
